...
async def register_user(db, tenant_name: str, email: str, password: str):
    try:
        tenant = Tenant(name=tenant_name)
        db.add(tenant)
        db.flush()
        user = User(email=email, password_hash=hash_password(password), tenant_id=tenant.id, role='owner')
        db.add(user)
        db.commit()
